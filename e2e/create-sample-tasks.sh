...
curl -s -X POST -H "Authorization: Bearer TaskGenToken" \
...
echo ""
echo "List all tasks:"
curl -H "Authorization: Bearer ClientToken" "$FLEA_URL/tasks?includeInactive=true"

echo ""
echo "List all ACTIVE tasks:"
curl -H "Authorization: Bearer ClientToken" $FLEA_URL/tasks
...
curl -H "Authorization: Bearer ClientToken" "$FLEA_URL/tasks?inludeInactive=true&maxItems=3"

echo ""
echo "List tasks starting from b7:"
curl -H "Authorization: Bearer ClientToken" "$FLEA_URL/tasks?includeInactive=true&startTaskId=b7"

echo ""
echo "List 2 tasks starting from task101:"
curl -H "Authorization: Bearer ClientToken" "$FLEA_URL/tasks?includeInactive=true&startTaskId=task101&maxItems=2"

echo ""
echo "Get details for task x3:"
curl -H "Authorization: Bearer ClientToken" $FLEA_URL/tasks/x3
...
echo ""
echo "Get details for task x3:"
curl -s  -H "Authorization: Bearer ClientToken" $FLEA_URL/tasks/x3

echo ""
echo "Reload credentials:"
curl -s  -X POST -H "Authorization: Bearer AdminToken" \
    -H "Content-Type: application/json" \
    -d "{\"type\": 1}" \
    $FLEA_URL/admin
...
echo ""
echo "Start task b7:"
curl -s  -H "Authorization: Bearer ClientToken" $FLEA_URL/start_task/b7
...
echo ""
echo "Unauthorized 1:"
curl -s  -H "Authorization: ClientToken" $FLEA_URL/start_task/b7
